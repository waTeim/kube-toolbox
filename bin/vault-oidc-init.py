...
def normalize_vault_addr(u: str) -> str:
    """VAULT_ADDR may be http or https; add scheme if missing, strip trailing slash."""
    u = (u or "").strip().rstrip("/")
    if not u:
        return u
...
def adopt_issuer_from_discovery(issuer_hint: str) -> str:
    """Fetch OIDC discovery and return the issuer EXACTLY as reported (keep trailing slash if present)."""
    u = issuer_hint.strip()
    if not urlparse(u).scheme:
        u = "https://" + u
    ...
    r.raise_for_status()
    disc_issuer = r.json().get("issuer", "")
    if not disc_issuer:
        raise SystemExit(f"No 'issuer' in discovery at {url}")
    return disc_issuer  # use verbatim; no rstrip
...
class Vault:
    ...
    def get(self, p: str):
        return self.session.get(self._u(p), verify=self.verify)
    def post(self, p: str, body: dict):
        return self.session.post(self._u(p), json=body, verify=self.verify)

def prompt_missing(a):
    def ask(msg, secret=False):
        return getpass.getpass(msg + ": ") if secret else input(msg + ": ").strip()
    if not a.vault_addr:
        a.vault_addr = ask("Vault address (e.g., https://vault.example.com)")
    if not a.vault_token and not a.vault_token_file:
        if (ask("Provide Vault token directly? [y/N]") or "n").lower().startswith("y"):
            a.vault_token = getpass.getpass("Vault token: ")
        else:
            a.vault_token_file = ask("Path to Vault token file (e.g., ~/.vault-token)")
    if a.vault_token_file and not a.vault_token:
        try:
            with open(os.path.expanduser(a.vault_token_file), "r") as f:
                a.vault_token = f.read().strip()
        except OSError as e:
            print(f"Failed to read token file: {e}", file=sys.stderr)
            sys.exit(1)
    if not a.issuer:
        a.issuer = ask("OIDC issuer base URL (e.g., https://your-idp.example.com)")
    if not a.client_id:
        a.client_id = ask("OIDC client ID")
    if not a.client_secret:
        a.client_secret = ask("OIDC client secret", secret=True)
...
def ensure_mount(v: Vault, mount: str):
    ...
    if key in mounts:
        t = mounts[key]["type"]
        print(f"[auth] mount '{mount}' exists (type={t}).")
        if t != "jwt":
            raise SystemExit(f"Auth mount '{mount}' is type '{t}', expected 'jwt'. Choose another --mount or remove it.")
        return
    print(f"[auth] enabling jwt at '{mount}' ...")
    r = v.post(f"sys/auth/{mount}", {"type": "jwt"})
    if r.status_code not in (200, 204):
        raise SystemExit(f"Enable auth failed [{r.status_code}]: {r.text}")
    print("[auth] enabled.")

def write_oidc_config(v: Vault, mount: str, issuer: str, client_id: str, client_secret: str, default_role: str, ca_pem: Optional[str]):
    # IMPORTANT: issuer is used verbatim (no trailing-slash manipulation)
    body = {
    ...
    if ca_pem:
        body["oidc_discovery_ca_pem"] = ca_pem
    print(f"[config] writing auth/{mount}/config ...")
    r = v.post(f"auth/{mount}/config", body)
    if r.status_code not in (200, 204):
        raise SystemExit(f"Write OIDC config failed [{r.status_code}]: {r.text}")
    print("[config] ok.")

def upsert_role(v: Vault, mount: str, role: str, vault_addr: str, policies_csv: str, groups_claim: Optional[str], scopes_csv: str, ttl: str):
    ui_cb = f"{vault_addr.rstrip('/')}/ui/vault/auth/{mount}/oidc/callback"
    scopes = [s.strip() for s in (scopes_csv or "").split(",") if s.strip()] or ["openid","profile","email"]
    # FIX: Ensure policies is a list, not a string
    policies = [p.strip() for p in (policies_csv or "").split(",") if p.strip()] or ["default"]
    body = {
        "role_type": "oidc",
        "user_claim": "email",
        "oidc_scopes": scopes,
        "allowed_redirect_uris": [ui_cb, "http://localhost:8250/oidc/callback"],
        "token_policies": policies,
        "token_ttl": ttl
    }
    if groups_claim:
        body["groups_claim"] = groups_claim
    print(f"[role] upserting auth/{mount}/role/{role} ...")
    r = v.post(f"auth/{mount}/role/{role}", body)
    if r.status_code not in (200, 204):
        raise SystemExit(f"Write role failed [{r.status_code}]: {r.text}")
    print("[role] ok.")

def main():
    ap = argparse.ArgumentParser(description="Minimal Vault OIDC setup (auth method + role) with short flags.")
    ap.add_argument("-a","--vault-addr", help="Vault address (also used for UI callback), e.g. https://vault.example.com")
    ap.add_argument("-t","--vault-token", help="Vault admin token (or use --vault-token-file)")
    ap.add_argument("-f","--vault-token-file", help="File containing Vault token (e.g., ~/.vault-token)")
    ...
    ap.add_argument("-m","--mount", default="oidc", help="Auth mount path (default: oidc)")
    ap.add_argument("-r","--role", default="devs", help="Role name (default: devs)")
    ap.add_argument("-p","--policies", default="default", help="Comma-separated token policies (default: default)")
    ap.add_argument("-i","--issuer", help="OIDC issuer base URL (e.g., https://your-idp.example.com). Use BASE, not /.well-known/…")
    ap.add_argument("-c","--client-id", help="OIDC client ID")
    ap.add_argument("-s","--client-secret", help="OIDC client secret")
    ...
    args.vault_addr = normalize_vault_addr(args.vault_addr)
    issuer_exact = adopt_issuer_from_discovery(args.issuer)  # keep trailing slash if present
    ...
    # write config (use issuer EXACTLY as discovery reports it)
    write_oidc_config(v, args.mount, issuer_exact, args.client_id, args.client_secret, args.role, ca_pem)

    # upsert role (policies now converted to a proper list)
