...
def discover_oidc_accessor(addr, token, oidc_path):
    ...
    if not accessor: raise SystemExit(f"[FATAL] Could not read accessor for auth mount '{oidc_path}'.")
...
""".strip()
    ...
    print(f"[OK] Wrote/updated policy '{policy_name}' (alias-aware, per-project)")
...
def maybe_attach_policy_to_oidc_role(addr, token, oidc_path, role, policy_name):
    ...
    if policy_name in existing:
        print(f"[OK] OIDC role '{role}' already includes policy '{policy_name}'"); return
...
def main():
    ...
    if not args.vault_token: raise SystemExit("[FATAL] No token: set VAULT_TOKEN or pass --vault-token.")
    ...
    if args.oidc_role: maybe_attach_policy_to_oidc_role(args.vault_addr, args.vault_token, args.oidc_path, args.oidc_role, args.policy_name)
